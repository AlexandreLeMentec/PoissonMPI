...
MODULE parallel
  ...
  ! Types derives
  INTEGER                                   :: typedp,type_ligne, type_colonne
  ! Requettes pour les communications non bloquantes
  INTEGER, DIMENSION(2*NB_VOISINS)          :: requete
  !Constantes MPI
  INTEGER                                   :: code
...
!                                code)
  ...
  SUBROUTINE fin_communication
    ...
    !Constantes MPI
    INTEGER, DIMENSION(MPI_STATUS_SIZE, 2*NB_VOISINS) :: tab_statut

    CALL MPI_WAITALL(2*NB_VOISINS, requete, tab_statut, code)
  ...
  SUBROUTINE ecrire_mpi(u)
    ...
    CALL MPI_TYPE_CREATE_SUBARRAY(rang_tableau, profil_tab, profil_sous_tab, coord_debut, &
     MPI_ORDER_FORTRAN, typedp, type_sous_tab, code)
    ...
    CALL MPI_TYPE_CREATE_SUBARRAY(rang_tableau, profil_tab_vue, profil_sous_tab_vue, coord_debut_vue, &
     MPI_ORDER_FORTRAN, typedp, type_sous_tab_vue, code)

    !Validation du type_derive type_sous_tab_vue
    CALL MPI_TYPE_COMMIT(type_sous_tab_vue, code)
    ...
    CALL MPI_TYPE_FREE(type_sous_tab,code)
    CALL MPI_TYPE_FREE(type_sous_tab_vue,code)
